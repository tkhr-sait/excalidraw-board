...
```
│ > Footer部分、自分のユーザ名のみ表示。ユーザ名はクリックで変更可能。コラボレーション参加者数は表示不要
│ > フッターの接続状態は不要。フッターはShare中のみ表示
│ > Footerは、RoomとUserのみ表示にしてください
│ > FooterのURLリンクは復活させてください
│ > フッターのフォントサイズを統一してください
│ > フッターのURLリンクはRoomIDのみで、ユーザ名は開いた先で必要に応じ生成してください
